    ...
    <>
      <div className='w-full max-w-5xl mx-auto z-20 max-md:px-4 min-h-[80vh]'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 flex-wrap gap-4'>
                        {plans.map((plan, idx) => (
                            <div key={idx} className={`relative p-6 bg-black/20 mx-auto w-full max-w-sm rounded-lg text-white shadow-lg transition-all duration-400 ${idx === 1 ? 'ring-2 ring-violet-500 hover:ring-violet-400' : 'ring ring-violet-950 hover:ring-violet-700'}`}>
                                {idx === 1 && (
                                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-600 text-white text-xs px-3 py-1 rounded-full">Most Popular</span>
                                )}
                                ...
                                <ul className="space-y-1.5 mb-6 text-sm">
                                    {plan.features.map((feature, i) => (
                                        <li key={i} className="flex items-center">
                                            <svg className="h-5 w-5 text-violet-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                ...
                                <button onClick={() => handlePurchase(plan.id)} className={`w-full py-2 px-4 active:scale-95 text-sm rounded-md transition-all ${idx === 1 ? 'bg-violet-600 hover:bg-violet-700' : 'bg-white/10 hover:bg-white/20'}`}>
                                    Get Started
                                </button>
                            </div>
                        ))}
