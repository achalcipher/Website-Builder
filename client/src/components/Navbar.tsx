...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-violet-900/40">
        <Link to='/'>
          ...
          {!session?.user ? (
            <button onClick={()=> navigate('/auth/signin')} className="px-6 py-1.5 max-sm:text-sm bg-violet-600 active:scale-95 hover:bg-violet-700 transition rounded">
              Start Building
            </button>
          ) : (
            <>
            <button className='bg-white/10 px-5 py-1.5 text-xs sm:text-sm border text-gray-200 rounded-full'>
            Credits : <span className='text-violet-300'>{credits}</span>
            </button>
